...
func TestEventRecorder_Event(t *testing.T) {
	...
	client.Init()
	logger := NewLogger().WithName("Test Event")
	eventRecorder := NewEventRecorder(client, logger)

	crdJsonBytes, err := client.GetResource("Pod", "default", "busybox")
	if err != nil {
		t.Error(err)
	}
	eventRecorder.Event([]byte(crdJsonBytes), EventTypeNormal, "test event", "this is a test")
}
